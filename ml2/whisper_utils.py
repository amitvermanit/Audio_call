...
class FasterWhisperASR(ASRBase):
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        #     model_size_or_path, 
        #     device="cuda", 
        #     compute_type="int8",        # Change from "int8_float16" to pure "int8" 
        #     cpu_threads=4,              # Add this - limits CPU overhead
        #     num_workers=1               # Add this - reduces parallel processing memory
        # )
...
#        model = WhisperModel(modelsize, device="cpu", compute_type="int8") #, download_root="faster-disk-cache-dir/")
    ...
    def transcribe(self, audio, init_prompt="The following professional transcription came from a serious business meeting about budgetting. Carefull and precise transcription was therefore essential. The scribe listened carefully and wrote down the following:"):
        ...
        #                                        initial_prompt=init_prompt, beam_size=5, word_timestamps=True, condition_on_previous_text=True, 
        #                                        **self.transcribe_kargs)
        #print(f"asr language: {info.language}")  # info contains language detection result
    ...
    def ts_words(self, segments):
        o = []
        for segment in segments:
            # Check segment quality BEFORE processing words
            print(f"no speech prrob : {segment.no_speech_prob}")
            if segment.no_speech_prob > 0.7:
                continue  # Skip entire segment
            
            # Add this crucial filter for hallucinations
            print(f"hallucination prob: {segment.avg_logprob}")
            if segment.avg_logprob is not None and segment.avg_logprob < -1.0:
                continue  # Skip low-confidence segments (catches repetitions)
                
            # Now process words from good segments only
            for word in segment.words:
                w = word.word
                t = (word.start, word.end, w)
                o.append(t)
